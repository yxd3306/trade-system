...
public class LockUtil {
    ...
    public static boolean unlock(String key, String value) {
        ...
        boolean success = LockJedisUtil.unlock(fullKey, value);
        if (success) {
            log.info("unlock success ; key:" + key + ",value:" + value);
        } else {
    ...
    private static boolean lock(String key, String value, int lockTime, boolean reTry, int curTryTime,
        ...
        log.info(Thread.currentThread().getName() + ",lock come in ; key:" + key + ",value:" + value
        ...
        // setnx 并设置超时时间
        boolean success = LockJedisUtil.setnx(fullKey, value, (long) lockTime * 1000);
        // 获取成功,直接返回
        if (success) {
            log.info("lock success ; key:" + key + ",value:" + value + ",lockTime:" + lockTime + ",reTry:"
        ...
        // 获取失败,不需要重试,直接返回
        if (!reTry) {
            log.info("lock failed ; key:" + key + ",value:" + value + ",lockTime:" + lockTime + ",reTry:"
                    + reTry + ",curTryTime:" + curTryTime + ",needTimeOut:" + needTimeOut + ",timeOutMillis:"
                    + timeOutMillis);
            return false;
        }
        ...
        // 大于100次,打印warning日志
        if (curTryTime > 100) {
            log.info("lock warning ; key:" + key + ",value:" + value + ",lockTime:" + lockTime + ",reTry:"
                    + reTry + ",curTryTime:" + curTryTime + ",needTimeOut:" + needTimeOut + ",timeOutMillis:"
                    + timeOutMillis);
        }
